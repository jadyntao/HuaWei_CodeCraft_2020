...
struct Dji_Block{
	...
	// vector<unsigned int> prv[max_id];
	pair<double,unsigned int>* sum;
	unsigned int *Stk;
};
...
void* dji(void* arg){
	...
	Dji_Block* dji_blk = (Dji_Block*)arg;
	pair<double,unsigned int>* sum  = dji_blk->sum;
	long long *dis = dji_blk->dis;
	double *delta = dji_blk->delta;
	double *sigma = dji_blk->sigma;
	unsigned int *Stk = dji_blk->Stk;
...
void solve(){
	...
	for(int i=0;i<Tnum;i++){
		...
		dji_blk->sum = (pair<double,unsigned int>*)calloc(max_id,sizeof(pair<double,unsigned int>));
		dji_blk->Stk = (unsigned int*)calloc(max_id,sizeof(unsigned int));
	}
	// cout<<"sum[0]:"<<sum[0]<<endl;
	// cout<<"sum + 0:"<<sum + 0<<endl;
	...
	for(unsigned int i=0;i<Tnum;i++)
		pthread_create(&Td[i],NULL,dji,(void*)(dji_block + i));
...
int main(){
	...
	string GraphFileName = num + "/graph2.txt";
	unsigned int MAX_OUT_DU = 0;
	unsigned int MAX_IN_DU = 0;
	for(int i = 0; i < max_id; i++){
		if(MAX_OUT_DU<out_du_num[i]) MAX_OUT_DU = out_du_num[i];
	...
	// 	}
	// }
	unsigned int t[MAX_IN_DU+1];
	for(int i=0;i<=MAX_IN_DU;i++)
		t[i]=0;
	for(int k=0;k<max_id;k++){
		t[in_du_num[k]]++;
	}
	for(int j=0;j<=MAX_IN_DU;j++){
		if(t[j]!=0){
			mys += " "+myto_string(j)+"    "+"x    "+myto_string(t[j])+"\n";
		}
	}

	unsigned int t2[MAX_OUT_DU+1];
	for(int i=0;i<=MAX_IN_DU;i++)
		t2[i]=0;
	for(int k=0;k<max_id;k++){
		t2[out_du_num[k]]++;
	...
	for(int j=0;j<=MAX_OUT_DU;j++){
		if(t2[j]!=0){
			mys += " x    "+myto_string(j)+"    "+myto_string(t2[j])+"\n";
		}
	}
