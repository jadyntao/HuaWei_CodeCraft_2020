...
void pushup(unsigned int* Min ,unsigned int* id ,unsigned int now)
{
	if(Min[now << 1] <= Min[now << 1 | 1]) Min[now] = Min[now << 1], id[now] = id[now << 1];
	else Min[now] = Min[now << 1 | 1], id[now] = id[now << 1 | 1];
}
void build(unsigned int* Min ,unsigned int* id,unsigned int* pos,unsigned int L,unsigned int R,unsigned int now)
{
	if(L == R) 
	{
	...
	build(Min,id,pos,L, mid, now << 1), build(Min,id,pos,mid + 1, R, now << 1 | 1);
	pushup(Min,id,now);
}
void update(unsigned int* Min ,unsigned int* id ,unsigned int now,unsigned int d)
{
	Min[now] = d;
	while(now >> 1) pushup(Min,id,now >> 1), now >>= 1;
}
...
void* dji(void* arg){
	...
	unsigned int *Min = (unsigned int*)calloc(max_id<<2,sizeof(unsigned int));
	unsigned int *id = (unsigned int*)calloc(max_id<<2,sizeof(unsigned int));
	unsigned int *pos = (unsigned int*)calloc(max_id<<2,sizeof(unsigned int));
	bool *vis = (bool*)calloc(max_id,sizeof(bool));
	...
	while(true){
		...
		prv[s].clear();
		// q.push((node){0,s});
		build(Min,id,pos,1, max_id, 1);
		update(Min,id,pos[1],dis[1]);
		Edge2 *cur;
		while(Min[1] ^ INF)
