...
inline void load_input(string &input_file){
	...
	*/
	...
	for(unsigned int i = 0; i < max_id; i++){
		...
		else if(finish[i]==max_id&&out_du_num[i]==1){
...
void dji(unsigned int s,int tid)
{
	...
	// cout<<1<<endl;
	// cnt[tid]++;
	// if(one_prv!=max_id) cnt_num[tid]+=2;
	// else cnt_num[tid]++;
    ...
    while(!Stk[tid].empty()){
		...
		if(one_prv!=max_id){
			...
			if(w!=one_prv){
				...
				// if(w==0) cout<<"!!"<<s<<" "<<delta[tid][w]<<endl;
			}
		}
		// if(s==4924&&one_prv==3930&&w==s)
		// 	cout<<w<<" "<<delta[tid][w]<<" -1"<<endl;
		if(w==s) continue;
		...
		// if(w==0) cout<<"!!"<<s<<" "<<delta2[tid][w]<<endl;	

		// if(s==4924&&one_prv==3930&&w==one_prv)
		// 	cout<<w<<" "<<"-1 "<<delta2[tid][w]<<endl;
		// else if(s==4924&&one_prv==3930)
