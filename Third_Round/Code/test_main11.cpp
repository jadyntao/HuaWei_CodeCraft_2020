...
void dji_not_dense(unsigned int s,int tid)
{
...
static void* dji_method_not_dense(void* c){
	int tid =*(int*)c;
	// cout<<1<<endl;
	while(1){
		pthread_mutex_lock(&mt);
...
int main(){
	...
	*/
	...
	if(MAX_OUT_DU>T){
		...
		cout<<"use dji_method_not_dense!"<<endl;
		for(unsigned int i=0;i<Tnum;i++)
		{
			cs[i]=i;
			pthread_create(&Td[i],NULL,dji_method_not_dense,(void*)(cs+i));
		}
		for(unsigned int i=0;i<Tnum;i++){
			// Td[i].join();
			pthread_join(Td[i],NULL);
