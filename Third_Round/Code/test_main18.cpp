...
void* dji(void* arg){
	...
	unsigned int **prv = (unsigned int**)malloc(max_id * sizeof(unsigned int*));
	for(unsigned int i=0;i<max_id;i++) 
		prv[i]= (unsigned int*)calloc(in_du_num[i],sizeof(unsigned int));
	...
	while(true){
		...
		while(sizeStk){
			...
			for(unsigned int i=0,v ;i<prv_size[w];i++){
				v = prv[w][i];
