...
inline void get_input(string &input_file){
	...
	for(int i=0;i<record_num;i++){
		int ids_in_num = ids2[inputs_in_num[i]];
		int ids_out_num = ids2[inputs_out_num[i]];
	...
	// 		total_offset += str[Graph[i][j].first].size();
	// 	total_offset += huanhang.size();
	// }
	// cout<<total_offset<<endl;
	// string Graph_file = "juesai/Graph1.txt";
	// int fd=open(Graph_file.c_str(),O_CREAT|O_RDWR,0666);
	// 1==ftruncate(fd,total_offset);
	// char*p=(char*)mmap(NULL,total_offset,PROT_WRITE|PROT_READ,MAP_SHARED,fd,0);
	// long long cur_offset = 0;
	// for(int i=1;i<node_num;i++){
	// 	for(int j=0;j<Graph[i].size();j++){
	// 		memcpy(p + cur_offset,str[Graph[i][j].first].c_str(),str[Graph[i][j].first].size());
	// 		cur_offset += str[Graph[i][j].first].size();
	// 	}
