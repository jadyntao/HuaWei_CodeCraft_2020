...
struct Dji_Block{
	...
	// Dji_Block(unsigned int i){
	// 	prv = (unsigned int*)calloc(in_du_num[i],sizeof(unsigned int));
	// }
};
...
void* dji(void* arg){
	...
	// Dji_Block *dji_block = (Dji_Block*)calloc(max_id,sizeof(Dji_Block));
	Dji_Block *dji_block = new Dji_Block[max_id];
	for(unsigned int i=0;i<max_id;i++) 
		(dji_block + i)->prv = (unsigned int*)calloc(in_du_num[i],sizeof(unsigned int));
	unsigned int s,sizeStk=0;
	// for(int i=0;i<max_id;i++) cout<<dji_block[i].dis<<endl;
	// cout<<"!!"<<INF<<endl;
	// unsigned int *prv_size =  (unsigned int*)calloc(max_id,sizeof(unsigned int));
	// cout<<"MAX_IN_DU:"<<MAX_IN_DU<<endl;
	// cout<<11<<endl;

	// unsigned int** prv = new unsigned int[max_id][10000];

	// unsigned int** prv = new unsigned int*[max_id];
	// for(int i=0;i<max_id;i++) prv[i] = new unsigned int[MAX_IN_DU];

	// unsigned int (*prv)[MAX_IN_DU] = new unsigned int[max_id][MAX_IN_DU];
	...
	while(true){
		...
		Dji_Block* dji_blk_s = dji_block+s;
		dji_blk_s->dis = 0;
		dji_blk_s->sigma = 1;
		dji_blk_s->prv_size = 0;
		// prv[s].clear();
		// prv_size[s] = 0;
		q.push((node){0,s});
		Edge2 *cur;
		while(!q.empty())//堆为空即为所有点都更新
		{
			...
			// cout<<s<<":"<<u<<","<<x.w<<endl;
			Dji_Block *dji_blk_u = dji_block+u;
			if(dji_blk_u->dis < x.w) continue; 
			Stk[sizeStk++] = u;
			if(isdense[u])
			...
			for(int i=0;i<len;i++)
			{
				...
				Dji_Block *dji_blk_v = dji_block+v;
				long long newDist=dji_blk_u->dis+cur[i].w;
				// cout<<s<<"::"<<dji_blk_u->dis<<" "<<newDist<<endl;
				if(dji_blk_v->dis>newDist)
				{
					dji_blk_v->dis=newDist;
					// cout<<s<<" "<<v<<endl;
					// prv_size[v]=0;
					// prv[v].clear();
					dji_blk_v->prv_size = 0;
					dji_blk_v->sigma=0;
					q.push((node){dji_blk_v->dis,v});
				}
				if(newDist==dji_blk_v->dis){
					// prv[v].emplace_back(u);
					// prv[v][prv_size[v]++] = u;
					dji_blk_v->prv[dji_blk_v->prv_size++] = u;
					dji_blk_v->sigma+=dji_blk_u->sigma;
				}
			}
		}
		...
		while(sizeStk){
			...
			Dji_Block *dji_blk_w = dji_block+w;
			double coeff2=(1.0+dji_blk_w->delta)/dji_blk_w->sigma;
			// for(unsigned int v:prv[w]){
			for(unsigned int v = 0,i=0;i<dji_blk_w->prv_size;i++){
				v = dji_blk_w->prv[i];
				Dji_Block *dji_blk_v = dji_block+v;
				dji_blk_v->delta += dji_blk_v->sigma*coeff2;
				// cout<<s<<" "<<w<<" "<<v<<endl;
			}   
			sum_[w].first += dji_blk_w->delta * one_prv;
			
				
		}
		sum_[s].first -= dji_blk_s->delta;
		for(unsigned int i=0;i<sizestk_;i++){
			unsigned int vi = Stk[i];
			dji_block[vi].dis = INF;
			dji_block[vi].delta = 0;
		}
