...
void IsLoop(unsigned int tid,unsigned int depth,int head, unsigned int v, unsigned int head_menoy,unsigned int cur_menoy,unsigned int next_menoy){
...
void dfs(unsigned int &cur ,unsigned int tid){
	...
	for(unsigned int i=0;i<size;i++){
		unsigned int v = Graph[cur][i].first;
		if(v<cur || first_one[tid][v]!=cur || visited[tid][v]) continue;
		unsigned int cur_menoy = Graph[cur][i].second;
...
void dfs_diedai(unsigned int &head ,unsigned int tid){
    ...
    path[tid][0] = head;
	long long r1,r2,r3,r4;
	unsigned int next_menoy, end_menoy,next_node;
    unsigned int size_0 = Graph[head].size();
	for(int a=0;a<size_0;a++){
		unsigned int v0 = Graph[head][a].first;
		if(v0<head || first_one[tid][v0]!=head || visited[tid][v0]) continue;
		unsigned int v0_menoy = Graph[head][a].second;
		...
		for(int b=0;b<size_1;b++){
			unsigned int v1 = Graph[v0][b].first;
			if(v1<head||visited[tid][v1]) 
				continue;
			unsigned int v1_menoy = Graph[v0][b].second;
			...
			for(int c=0;c<size_2;c++){
				unsigned int v2 = Graph[v1][c].first;
				if(v2<head||visited[tid][v2]) 
					continue;
				unsigned int v2_menoy = Graph[v1][c].second;
				...
				for(int d=0;d<size_3;d++){
					unsigned int v3 = Graph[v2][d].first;
					if(v3<head||visited[tid][v3]) 
						continue;
					unsigned int v3_menoy = Graph[v2][d].second;
					...
					for(int e=0;e<size_4;e++){
						unsigned int v4 = Graph[v3][e].first;
						if(v4<head||visited[tid][v4]) 
							continue;
						unsigned int v4_menoy = Graph[v3][e].second;
						...
						if(two_border_head[tid][v4] == head ){
							...
							for(int j=0;j<v_size;j++){
								unsigned int next_menoy = two_border[tid][v4][j].menoy2;;
								unsigned int end_menoy = two_border[tid][v4][j].menoy1;
								unsigned int next_node = two_border[tid][v4][j].v;
								if(!visited[tid][next_node]){
									r1 = next_menoy * 5ll;
									r4 = end_menoy * 3ll;
...
void dfs_cut_backward(unsigned int &head,unsigned int tid){
    ...
    for(int i=0;i<len1;i++){
		...
		for(int fi=0;fi<flen1;fi++){
			unsigned int &fv1=Graph[head][fi].first;
			// if(first_one[tid][fv1]==head) continue;
			if(fv1<=head)continue;
			unsigned int &fv1_money=Graph[head][fi].second;
			long long fr1 = fv1_money * 5ll;
			if(fr1<v1_money||fv1_money>r1) continue;
			first_one[tid][fv1] = head;
			// flag = true;
		}
		// if(flag==true) three_border[tid][v1] = head;
