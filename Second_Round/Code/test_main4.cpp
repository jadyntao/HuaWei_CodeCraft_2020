...
void dfs_cut_backward(unsigned int &head,unsigned int &cur,int depth,unsigned int tid,unsigned int &label){
	visited[tid][cur] = true;
...
static void* dfs_method(void* c){
	...
	while(1){
		...
		if(size1[cur]){
			...
			dfs_cut_backward(cur,cur,0,tid,label);
