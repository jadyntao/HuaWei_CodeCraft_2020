...
void dfs2(unsigned int &head,unsigned int &cur,int depth,unsigned int tid,unsigned int&last_menoy,unsigned int&head_menoy){
	...
	for(int i=0;i<size;i++){
    	...
    	if( lastone[tid][v].first == head && depth ){
			...
			r1 = head_menoy * 5ll;
			r2 = next_menoy * 3ll;
			long long r3 = next_menoy * 5ll;
			long long r4 = cur_menoy * 3ll;
			if(r1>=next_menoy&&head_menoy<=r2&&r3>=cur_menoy&&next_menoy<=r4){
...
void dfs(unsigned int &head,unsigned int &cur,int depth,unsigned int tid){
    visited[tid][cur]=true;
    path[tid][depth] = cur;
    unsigned int size = Graph[cur].size();
	for(int i=0;i<size;i++){
        ...
        dfs2(head,v,depth+1,tid,cur_menoy,cur_menoy);
    }
    visited[tid][cur]=false;
}
void dfs_cut_backward(unsigned int &head,unsigned int tid){
    ...
    for(int i=0;i<len1;i++){
		...
		lastone[tid][v1].second=Graph2[head][i].second;
        long long r1 = Graph2[head][i].second * 5ll;
        // long long r1 = cur_menoy * 5ll;
		// long long r2 = last_menoy * 3ll;
		// if(r1<last_menoy||cur_menoy>r2) continue;
		//depth = 1  -> v1
		...
		for(int j=0;j<len2;j++){
            ...
            long long r2 = Graph2[v1][j].second*3ll;
            if(r1<Graph2[v1][j].second||Graph2[head][i].second>r2) continue;
            r2 = Graph2[v1][j].second*5ll;
			...
			for(int k=0;k<len3;k++){
                ...
                long long r3 = Graph2[v2][k].second*3ll;
                if(r2<Graph2[v2][k].second||Graph2[v1][j].second>r3) continue;
