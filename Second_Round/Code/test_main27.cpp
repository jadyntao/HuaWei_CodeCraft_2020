...
void add_g_g2(unsigned int u,unsigned int v,long long w){
    Graph[tot].from=u;
    Graph[tot].to=v;
    Graph[tot++].money=w;
...
inline void get_input(string &input_file){
	...
	for(int i=0;i<record_num;i++){
		...
		long long menoy = inputs_menoy[i];
...
void dfs5(unsigned int &head,unsigned int &cur,unsigned int tid,long long&last_menoy,long long&head_menoy){
    // cnt[tid]++;
	...
	for(unsigned int  i=fir[cur];Graph[i].from==cur;i++){
		note *v_ptr = &Graph[i];
		unsigned int v = v_ptr->to;
		if(v<head || visited[tid][v]) continue;
		long long cur_menoy = v_ptr->money;
		...
		if(two_border_head[tid][v] == head ){
			...
			for(int j=0;j<v_size;j++){
				...
				b_ptr = &two_border[tid][v][j];
				unsigned int next_node = b_ptr->v;
				if(!visited[tid][next_node]){
					long long next_menoy = b_ptr->menoy2;
					long long end_menoy = b_ptr->menoy1;
...
void dfs4(unsigned int &head,unsigned int &cur,unsigned int tid,long long&last_menoy,long long&head_menoy){
    // cnt[tid]++;
	visited[tid][cur]=true;
    path[tid][4] = cur;
	long long r1,r2,r3,r4;
	...
	for(unsigned int  i=fir[cur];Graph[i].from==cur;i++){
		...
		if(three_border[tid][v]==head)
			dfs5(head,v,tid,cur_menoy,head_menoy);
    }
    visited[tid][cur]=false;
}


void dfs3(unsigned int &head,unsigned int &cur,unsigned int tid,long long&last_menoy,long long&head_menoy){
    // cnt[tid]++;
	visited[tid][cur]=true;
    path[tid][3] = cur;
	long long r1,r2,r3,r4;
...
void dfs2(unsigned int &head,unsigned int &cur,unsigned int tid,long long&last_menoy,long long&head_menoy){
    // cnt[tid]++;
	visited[tid][cur]=true;
    path[tid][2] = cur;
	long long r1,r2,r3,r4;
...
void dfs1(unsigned int &head,unsigned int &cur,unsigned int tid,long long&last_menoy,long long&head_menoy){
    // cnt[tid]++;
	visited[tid][cur]=true;
    path[tid][1] = cur;
	long long r1,r2,r3,r4;
	...
	for(unsigned int  i=fir[cur];Graph[i].from==cur;i++){
		...
		if(two_border_head[tid][v] == cur){
			...
			// for(int j=0;j<two_border[tid][v].size();j++){
				node *b_ptr = &two_border[tid][v][j];
				long long next_menoy = b_ptr->menoy2;
				long long end_menoy = b_ptr->menoy1;
				r1 = next_menoy * 5ll;
				r4 = end_menoy * 3ll;
				// if(path[tid][0]==5019&&v==5020)
				// 	cout<<cur_menoy<<" "<<next_menoy<<" "<<end_menoy<<" "<<endl;
				// if(next_menoy<0.2*cur_menoy||next_menoy>3.0*cur_menoy)	continue;
				// if(head_menoy<0.2*end_menoy||head_menoy>3.0*end_menoy)   continue;
...
void dfs_cut_backward(unsigned int &head,unsigned int tid){
	for(unsigned int  i=fir2[head];Graph2[i].from==head;i++){
		note *v1_ptr = &Graph2[i];
    	unsigned int v1=v1_ptr->to;
		if(v1<=head)continue;
		long long v1_money=v1_ptr->money;
		// three_border[tid][v1] = head;
		// unsigned int *three_border_ptr = three_border[tid];
		// // *(three_border_ptr+v1) = head;
