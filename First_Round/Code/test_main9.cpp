...
//					last_one[v] = head;
	...
	void dfs_method(){
		...
		three_border = vector<int>(node_num,-1);
		last_one = vector<int>(node_num,-1);
		for(unsigned int i=0;i<node_num;++i){
			if(!Graph[i].empty()){
//				dfs_cut_forward(i,i,1);
				dfs_cut_backward(i,i,1);
				for(unsigned int &v:Graph2[i]){
					last_one[v] = i;
				}
...
int main(){
	string num_str = "1004812.txt";
	string InputFileName = "test_data" + num_str;
	string OutputFileName = "output" + num_str;
	string ResultFileName = "result" + num_str;
	auto time = 0;
	int test_nums = 1;
