...
inline bool limit(uint64_t w1, uint64_t w2) {
    return w2 * 5ul >= w1 && w2 <= w1 * 3ul;
    /*
...
struct Result {
    ...
    inline void insert4(const ui &id, const ui &x, const ui &y, const ui &z) {
        memcpy(str+strLen, nodePreListStr[id].str, nodePreListStr[id].len);
        strLen += nodePreListStr[id].len;
        memcpy(str+strLen, nodePreListStr[x].str, nodePreListStr[x].len);
        strLen += nodePreListStr[x].len;
        memcpy(str+strLen, nodePreListStr[y].str, nodePreListStr[y].len);
        strLen += nodePreListStr[y].len;
        memcpy(str+strLen, nodePreListStr[z].str, nodePreListStr[z].len);
        strLen += nodePreListStr[z].len;
        str[strLen - 1] = '\n';
    }

    inline void insert5678(char *start, const int len, const Path &path) {
        ...
        memcpy(str+strLen, nodePreListStr[path.stp3].str, nodePreListStr[path.stp3].len);
        strLen += nodePreListStr[path.stp3].len;
        str[strLen-1] = '\n';
    }
...
InputFormat *inputTemp[THREADNUM];
...
inline InputFormat *readInputData(const ui i) {
    if (i < dataSum[0])
...
void *readLineThread(void *threadId) {
    ...
    //stop = inputStrLen;
    dataNum[id] = 0;
    inputTemp[id] = (InputFormat *) (malloc(sizeof(InputFormat) * SIXTYTHOUSAND));
    if (id == THREADNUM - 1)
        stop = inputStrLen;
    else
        stop = readStart[id + 1];
    for (int i = 0; p < stop; i++) {
        ...
        uint64_t weight = 0;
        while (inputStr[p] != ',') {
            start = start*10 + (inputStr[p] - '0');
            p++;
        }
        inputTemp[id][i].start = start;
        p++;
        while (inputStr[p] != ',') {
            end = end*10 + (inputStr[p] - '0');
            p++;
        }
        inputTemp[id][i].end = end;
        p++;
        while (inputStr[p] != '\n' && inputStr[p] != '\r' && inputStr[p] != '.') {
            weight = weight * 10 + (inputStr[p] - '0');
            p++;
        }
        int decimal = 0;
        ...
        if(inputStr[p] == '.'){
            p++;
            if(inputStr[p] != '\n' && inputStr[p] != '\r'){
                decimal = decimal * 10 + (inputStr[p] - '0');
                p++;
            }
            if(inputStr[p] != '\n' && inputStr[p] != '\r'){
                decimal = decimal * 10 + (inputStr[p] - '0');
        ...
        inputTemp[id][i].weight = weight*100 + decimal;
        //cout << inputTemp[id][i].start << ","<< inputTemp[id][i].end <<","<< inputTemp[id][i].weight << endl;
        while (inputStr[p] == '\n' || inputStr[p] == '\r')
            p++;
        if(inputTemp[id][i].weight == 0){
            inputTemp[id][i].start = 0;
            inputTemp[id][i].end  = 0;
            i--;
            continue;
        }
        dataNum[id]++;
...
void *treat_thread(void *arg){
    ...
    for (int i = front; i < back; i++) {
        ...
        InputFormat *edge = readInputData(i);
        ui start = MapGet(nodeIDMap,edge->start);
        ui end = MapGet(nodeIDMap,edge->end);
...
inline void treatData() {
    ...
    for (int i = 0; i < totalDataNum; i++) {
        const InputFormat *edge = readInputData(i);
        const ui start = edge->start;
        const ui end = edge->end;
...
void *search_thread(void *arg) {
    ...
    struct PathList pathList[120000];
    Result *r = result[threadId];
    r[0].str = (char *) (malloc(sizeof(char) * 11u * (0u + 1u) * (10000000u * (0u + 1u))));
    r[1].str = (char *) (malloc(sizeof(char) * 11u * (1u + 1u) * (10000000u * (1u + 1u))));
    r[2].str = (char *) (malloc(sizeof(char) * 11u * (2u + 1u) * (10000000u * (2u + 1u))));
    r[3].str = (char *) (malloc(sizeof(char) * 11u * (3u + 1u) * (10000000u * (3u + 1u))));
    r[4].str = (char *) (malloc(sizeof(char) * 11u * (4u + 1u) * (10000000u * (4u + 1u))));
    r[5].str = (char *) (malloc(sizeof(char) * 11u * (5u + 1u) * (10000000u * (5u + 1u))));
    ...
    while(true){
        ...
        for (int id = begin; id < end; id++) {
            ...
            for (Edge *ix = xIter.next(); ix; ix = xIter.next()) {
                ...
                if (x.vertice <= id)
                    continue;
                NodeIter yIter(getInNodeSet(x.vertice));
                for (Edge *iy = yIter.next(); iy; iy = yIter.next()) {
                    const Edge y = *iy;
                    if (y.vertice <= id || !limit(y.weight, x.weight))
                        continue;
                    NodeIter zIter(getInNodeSet(y.vertice));
                    for (Edge *iz = zIter.next(); iz; iz = zIter.next()) {
                        const Edge z = *iz;
                        if (z.vertice <= id || z.vertice == x.vertice || !limit(z.weight, y.weight))
                            continue;
                        NodeIter DIter(getInNodeSet(z.vertice));
                        for (Edge *iD = DIter.next(); iD; iD = DIter.next()){
                            const Edge D = *iD;
                            if(D.vertice <= id || D.vertice == x.vertice || D.vertice == y.vertice || !limit(D.weight, z.weight)){
                                continue;
                            }
                            int *head = &pathListHead[D.vertice];
                            Path p = {x.vertice, y.vertice, z.vertice, x.weight, D.weight};
                            if (isThirdNode[D.vertice] != id) {
                                isThirdNode[D.vertice] = id;
                                pathList[pointer].next = -1;
                                //cout << z <<endl;
                            } else {
                                for (; *head != -1; head = &pathList[*head].next) {
                                    __builtin_prefetch(&pathList[pathList[*head].next], 0);
                                    Path *pl = &pathList[*head].p;
                                    if (p.stp1 < pl->stp1 || (p.stp1 == pl->stp1 && p.num < pl->num)) {
                                        break;
                                    }
                                }
                ...
                if (isThirdNode[x.vertice] == id) {
                    ...
                    for (int I = pathListHead[x.vertice]; I != -1; I = pathList[I].next) {
                        ...
                        if (limit(x.weight, path->win) && limit(path->wout, x.weight)) {
                            ...
                            r[2].insert5678(routeStr, p, *path);
                            resultLen[threadId]++;
                        }
                    }
                }
                ...
                for (Edge *iy = yIter.next(); iy; iy = yIter.next()) {
                    ...
                    if (isThirdNode[y.vertice] == id) {
                        ...
                        for (int I = pathListHead[y.vertice]; I != -1; I = pathList[I].next) {
                            __builtin_prefetch(&pathList[pathList[I].next], 0);
                            const Path *path = &pathList[I].p;
                            if (x.vertice != path->stp1 && x.vertice != path->stp2 && x.vertice != path->stp3 &&
                                limit(y.weight, path->win) && limit(path->wout, x.weight)) {
                                __builtin_prefetch(&nodePreListStr[path->stp2], 0);
                                __builtin_prefetch(&nodePreListStr[path->stp1], 0);
                                r[3].insert5678(routeStr, p, *path);
                                resultLen[threadId]++;
                            }
                        }
                    }
                    ...
                    for (Edge *iz = zIter.next(); iz; iz = zIter.next()) {
                        ...
                        if (z.vertice <= id || x.vertice == z.vertice || !limit(y.weight, z.weight)) continue;
                        //route[3] = z.vertice;
                        // 检查是否有7环
                        if (isThirdNode[z.vertice] == id ) {
                            ...
                            for (int I = pathListHead[z.vertice]; I != -1; I = pathList[I].next) {
                                ...
                                const Path *path = &pathList[I].p;
                                if (x.vertice != path->stp1 && x.vertice != path->stp2 && x.vertice != path->stp3 &&
                                    y.vertice != path->stp1 && y.vertice != path->stp2 && y.vertice != path->stp3 &&
                                    limit(z.weight, path->win) && limit(path->wout, x.weight) ) {
                                    __builtin_prefetch(&nodePreListStr[path->stp2], 0);
                                    __builtin_prefetch(&nodePreListStr[path->stp1], 0);
                                    r[4].insert5678(routeStr, p, *path);
                                    resultLen[threadId]++;
                                }
                            }
                        }
                        ...
                        for (Edge *iD = DIter.next(); iD; iD = DIter.next()) {
                            const Edge D = *iD;
                            // 检查是是否有4环
                            if (id == D.vertice && limit(D.weight, x.weight) && limit(z.weight, D.weight)) {
                                r[1].insert4(id, x.vertice, y.vertice, z.vertice);
                                resultLen[threadId]++;
                                continue;
                            }
                            if (D.vertice <= id|| x.vertice == D.vertice || y.vertice == D.vertice || !limit(z.weight, D.weight) || pointer == 0)
                                continue;
                            ...
                            if (isThirdNode[D.vertice] == id) {
                                ...
                                for (int I = pathListHead[D.vertice]; I != -1; I = pathList[I].next) {
                                    ...
                                    if (x.vertice != path->stp1 && x.vertice != path->stp2 && x.vertice != path->stp3 &&
                                        y.vertice != path->stp1 && y.vertice != path->stp2 && y.vertice != path->stp3 &&
                                        z.vertice != path->stp1 && z.vertice != path->stp2 && z.vertice != path->stp3 &&
                                        limit(D.weight, path->win) && limit(path->wout, x.weight)) {
                                        __builtin_prefetch(&nodePreListStr[path->stp2], 0);
                                        __builtin_prefetch(&nodePreListStr[path->stp1], 0);
                                        r[5].insert5678(routeStr, p, *path);
                                        resultLen[threadId]++;
                                    }
                                }
                            }
...
# define OUTPUTSIZE sizeof(char) * totalLen * (32*3+3)
inline void outputResult(char *outputPath) {
...
int main() {
    ...
    /*
    char inputPath[] = "../data/fusai/test_data.txt";
    char outputPath[] = "../output/result.txt";
    char answerPath[] = "../data/fusai/result.txt";
    // */
    //*
    char inputPath[] = "../data/3512444/test_data.txt";
    char outputPath[] = "../output/result.txt";
    char answerPath[] = "../data/3512444/result.txt";
    // */
    /*
